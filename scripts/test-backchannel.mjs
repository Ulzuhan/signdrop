...
const EMISOR = `${ORIGEN_IDP}/application/o/signdrop`;
const CLIENT_ID = process.env.CLIENT_ID ?? "signdrop-pruebas";
...
check("de otro emisor", (await avisar(firmar({ carga: { iss: `${ORIGEN_IDP}/otro` } }))).status, 400);
...
const ida = await fetch(`${BASE}/api/auth/login?next=%2F`, { redirect: "manual" });
guardar(ida);
...
// SignDrop no tiene un "quién soy" ni un listado: lo que exige sesión es
// emitir una invitación, que además es la puerta que la decisión 0.1 añadió.
const puede = async () =>
  (
    await fetch(`${BASE}/api/guest-links`, {
      method: "POST",
      headers: { cookie: cabecera(), "content-type": "application/json" },
      body: JSON.stringify({ label: "prueba", ttlHours: 1 }),
    })
  ).status;
...
check("la sesión YA NO sirve: eso es la revocación llegando", await puede(), 401);

// Y lo que NO puede pasar: que revocar a uno eche a los demás.
const otroSub = `otro-${randomUUID()}`;
