...
export async function sealPdfDocument(options: SealPdfOptions): Promise<{
  ...
  if (includeAuditSheet) {
    ...
    // Page footer. The host is whatever origin produced this sheet, never a
    // name written into the source: this is an MIT repository, and a copy of
    // it running somewhere else must not print ours on a stranger's contract.
    const footerHost = (() => {
      try {
        return auditData.verificationUrl ? new URL(auditData.verificationUrl).host : null;
      } catch {
        return null;
      }
    })();
    auditPage.drawText(`Generated by SignDrop${footerHost ? ` • ${footerHost}` : ''}`, {
      x: 40,
      y: 40,
      size: 8,
      font: fontHelvetica,
